...
class flatten(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        for i in feat:
            output.append(np.ndarray.flatten(i))
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        for d in dprev:
            dfeat.append(d.reshape(feat.shape[1:]))
        dfeat=np.asarray(dfeat)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class fc(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        inp=feat.reshape(feat.shape[0],self.input_dim)
        weights=self.params[self.w_name]
        bias=np.transpose(self.params[self.b_name])
        output= np.dot(inp,weights)
    ...
    def backward(self, dprev):
        ...
        assert len(feat.shape) == 2 and feat.shape[-1] == self.input_dim, \
            "feat But got {} and {}".format(feat.shape, self.input_dim)
        assert len(dprev.shape) == 2 and dprev.shape[-1] == self.output_dim, \
            "dprev But got {} and {}".format(dprev.shape, self.output_dim)
        #############################################################################
        ...
        trans=feat.T
        self.grads[self.w_name]=np.matmul(trans,dprev)
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training:
            if self.keep_prob==0:
                ...
                feat=np.divide(feat,self.keep_prob)
                kept=self.rng.binomial(1,self.keep_prob,np.prod(feat.shape)).reshape(feat.shape)
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        if self.keep_prob>0:
            dprev=np.divide(dprev,self.keep_prob)
...
class cross_entropy(object):
    ...
    def backward(self):
        ...
        if self.size_average==True:
            for i,lbl in enumerate(label):
                dlogit[i][lbl]=dlogit[i][lbl]-1
        dlogit=np.divide(dlogit,len(self.logit))
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.logit = None
...
def softmax(feat):
    ...
    #############################################################################
    for f in feat:
        sum_s=np.sum(np.exp(f))
        temp=[]
        for ele in f:
            ele=np.exp(ele)/sum_s
            temp.append(ele)
        scores.append(temp)
